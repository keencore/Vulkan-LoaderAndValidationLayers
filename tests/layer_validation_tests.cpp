...
TEST_F(VkLayerTest, SimultaneousUse) {
    ...
    const char *cannot_free_descriptorsets_message =
            "Cannot call vkFreeDescriptorSets() on descriptor set ";
    ...
    m_errorMonitor->SetDesiredFailureMsg(VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                         cannot_free_descriptorsets_message);
    vkFreeDescriptorSets(m_device->device(), descriptorset_pool, 1,
                         &descriptorset);
    m_errorMonitor->VerifyFound();
    ...
    vkQueueSubmit(m_device->m_queue, 1, &submit_info, nullptr);
    vkBeginCommandBuffer(m_commandBuffer->handle(), &command_buffer_begin_info);
    m_errorMonitor->SetDesiredFailureMsg(VK_DEBUG_REPORT_ERROR_BIT_EXT,
